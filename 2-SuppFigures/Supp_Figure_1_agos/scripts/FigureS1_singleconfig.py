...
parser.add_argument('-rX',      '--rX',     help='rX',     type=float, default=1, required=False)
parser.add_argument('-J',       '--J',      help='J',      type=float, default=0.0004, required=False)
parser.add_argument('-nrep',    '--nrep',   help='nrep',   type=int,   default=2, required=False)
parser.add_argument('-tensor',  '--tensor', help='tensor', type=bool,  default=False, required=False)
...
T=np.arange(0,Tmax_over_tau_E*fun.tau_E,1e-4)#fun.tau_I/3);
T_tensor=torch.arange(0,Tmax_over_tau_E*fun.tau_E,1e-4).to(device)#fun.tau_I/3).to(device)
...
RATES=-1*np.ones((len(fun_vecs_E),len(L),N_E+N_I))
moments_of_r_sim=np.zeros((5,len(fun_vecs_E)))
...
#         print((idx_r_X+1)/len(r_X),(idx_L+1)/len(L))
        if tensor:
            DYNA, MUS,Lambda_i_L=fun.High_dimensional_dynamics_tensor(T_tensor,L_local,r_X,M_tensor,
                mu_X_over_r_X_tau_tensor,Lambda_i_tensor,E_cond,func_E,func_I)
        else:
            DYNA, MUS,Lambda_i_L=fun.High_dimensional_dynamics(T,L_local,r_X,M,mu_X_over_r_X_tau,
                Lambda_i,N_E,N_I,func_E,func_I);
        RATES[idx_funcs,idx_L,:]=np.mean(DYNA,axis=1)
    #------------------------------------------------

    mask_active_cells=(RATES[idx_funcs,1,:]>=0)|(RATES[idx_funcs,0,:]>=0)
    Base_Sim=RATES[idx_funcs,0,:][mask_active_cells]
    Delta_Sim=(RATES[idx_funcs,1,:]-RATES[idx_funcs,0,:])[mask_active_cells]

    moments_of_r_sim[0,idx_funcs]=np.mean(Base_Sim)
    moments_of_r_sim[1,idx_funcs]=np.mean(Delta_Sim)
    moments_of_r_sim[2,idx_funcs]=np.std(Base_Sim)
    moments_of_r_sim[3,idx_funcs]=np.std(Delta_Sim)
    moments_of_r_sim[4,idx_funcs]=np.cov(Base_Sim,Delta_Sim)[0,1]/(np.std(Delta_Sim)**2)
...
nameout='FigureS1_nrep='+str(nrep)+name_end
