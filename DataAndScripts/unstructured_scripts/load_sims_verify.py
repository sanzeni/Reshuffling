...
validatedir = "./validation_results/"
...
results_aux, jn = su.get_concatenated_results(resultsdir)
    

results=su.return_filtered_results(results_aux,filter)

params_fixed = results[0,list(su.res_param_idxs_fixed.values())]
sim_params = results[:,list(su.res_param_idxs.values())]
sim_moments = results[:,list(su.res_moment_idxs.values())]
...
p = params_fixed[1]
Lam = params_fixed[2]
Tmax_over_tau_E = params_fixed[3]
...
for seed_idx,seed_con in enumerate(seeds):
    ...
    net=network.network(seed_con=int(seed_con), n=2, Nl=25, NE=8, gamma=0.25, dl=1, ori_type=ori_type)

    net.generate_disorder(J,gE,gI,beta,p,CV_K,rX,K,Lam,CV_Lam,50.0,vanilla_or_not=True)
    this_moments,_,_ = fun.get_moments_of_r_sim(net,ri,T,mask_time,L,False,max_min)
...
if this_loss < cut_sim_loss:
    ...
    for rX_idx,this_rX in enumerate(rXs):
        ...
        for seed_idx,seed_con in enumerate(seeds):
            ...
            net.generate_disorder(J,gE,gI,beta,p,CV_K,this_rX,K,Lam,CV_Lam,50.0,vanilla_or_not=True)
            this_moments,_,_ = fun.get_moments_of_r_sim(net,ri,T,mask_time,L,False,max_min)
