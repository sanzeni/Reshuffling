...
pdf = pd.read_csv('simulation_param_Fixed_gs.txt',delim_whitespace=True) #header=None
print(args)
...
print(idx_species,g_E,g_I,nRep)
...
if idx_species < 2:
    ...
    f_handle = open('perceptron_results/results_Fixed_gs_'+['mice','monkeys'][idx_species]+'.txt','w')
    ...
    print('fit simulations to data')
    sol,cost=fun.fit_model_to_data_both_species_fixed_gs(g_E,g_I,dataset,Predictor_data,nCon,nRep,param_min,param_max)
    ...
    # Clean file to print results
    f_handle = open('perceptron_results/results_Fixed_gs_'+'both_species'+'.txt','w')
    np.savetxt(f_handle,results,fmt='%.6f', delimiter='\t')
    f_handle.close()
