...
CV_K=pdf.CV_K.iloc[jn]; 
idx_species=int(pdf.idx_species.iloc[jn]);
idx_combined=int(pdf.idx_combined.iloc[jn]);
...
print(sigma_t,tau_rp,g_E,g_I,beta,K,p,sigma_X_over_r_X,
      Lambda,sigma_Lambda_over_Lambda,J,r_X,ell,Tmax_over_tau_E,CV_K,idx_species,idx_combined)
...
idx_r_X,idx_L=0,-1
#mask_active_cells=(RATES[idx_r_X,idx_L,:]>=1)|(RATES[idx_r_X,0,:]>=1)
mask_active_cells=(RATES[idx_r_X,idx_L,:]>=0)|(RATES[idx_r_X,0,:]>=0)
Base_Sim=RATES[idx_r_X,0,:][mask_active_cells]
Delta_Sim=(RATES[idx_r_X,idx_L,:]-RATES[idx_r_X,0,:])[mask_active_cells]
...
sim_param=np.asarray([sigma_t,tau_rp,g_E,g_I,beta,K,p,CV_K,1,
...
cases=[idx_species,idx_combined]
...
np.savetxt(f_handle,results[0,:],fmt='%.6f', delimiter='\t')
f_handle.close()
