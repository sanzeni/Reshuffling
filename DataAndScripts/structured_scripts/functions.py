...
def get_moments_of_r_sim(net,ri,T,mask_time,L,RF,tuned,return_activity=True,max_min=7.5, return_dynas=False):
    if tuned=='yes':
    ...
    MFT_SOL_R_A=np.ones((2,10))
    MFT_SOL_R_T=np.ones((2,10))
    MFT_SOL_M_A=np.ones((2,10))
    MFT_SOL_M_T=np.ones((2,10))
    ...
    mask_tune_active=(RATES[0,tune_idx]>=0)|(RATES[1,tune_idx]>=0)
    Base_Sim_Tune=RATES[0,tune_idx][mask_tune_active]
    Delta_Sim_Tune=(RATES[1,tune_idx]-RATES[0,tune_idx])[mask_tune_active]

    moments_of_r_sim=np.zeros(10)

    if base_timeout:
        moments_of_r_sim[0]=1e6
        moments_of_r_sim[5]=1e6
        moments_of_r_sim[2]=1e6
        moments_of_r_sim[7]=1e6
    else:
        moments_of_r_sim[0]=np.mean(Base_Sim)
        moments_of_r_sim[5]=np.mean(Base_Sim_Tune)
        moments_of_r_sim[2]=np.std(Base_Sim)
        moments_of_r_sim[7]=np.std(Base_Sim_Tune)

    if opto_timeout:
        moments_of_r_sim[1]=1e6
        moments_of_r_sim[6]=1e6
        moments_of_r_sim[3]=1e6
        moments_of_r_sim[8]=1e6
        moments_of_r_sim[4]=1e6
        moments_of_r_sim[9]=1e6
    else:
        moments_of_r_sim[1]=np.mean(Delta_Sim)
        moments_of_r_sim[6]=np.mean(Delta_Sim_Tune)
        moments_of_r_sim[3]=np.std(Delta_Sim)
        moments_of_r_sim[8]=np.std(Delta_Sim_Tune)
        moments_of_r_sim[4]=np.cov(Base_Sim,Delta_Sim)[0,1]
        moments_of_r_sim[9]=np.cov(Base_Sim_Tune,Delta_Sim_Tune)[0,1]
...
def high_dimensional_dynamics(ri,T,L,M,H,LAM,Einds,Iinds,max_min=7.5,mult_tau=True):
